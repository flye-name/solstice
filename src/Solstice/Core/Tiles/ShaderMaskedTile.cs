...
// borrowed from a pull request by Lucille Karma
// TODO: either removed when the PR is merged, or refactor before release
namespace Solstice.Core.Tiles;
...
public abstract class ShaderMaskedTile : ModTile
{
    ...
    private void RenderShadedMaskTargetResults()
    {
        ...
        Color outlineColor = new(210, 163, 81);

        ApplyShader();

        Main.spriteBatch.Draw(texture, position + new Vector2(-2, 0), outlineColor);
        Main.spriteBatch.Draw(texture, position + new Vector2(2, 0), outlineColor);
        Main.spriteBatch.Draw(texture, position + new Vector2(0, -2), outlineColor);
        Main.spriteBatch.Draw(texture, position + new Vector2(0, 2), outlineColor);
